...
const CHECK_META: Record<string, { label: string; tip: string }> = {
  admin:    { label: "?????",           tip: "???????????????????? Windows Defender ?????????????" },
  webview2: { label: "Windows ???????", tip: "OpenClaw ???? Edge/WebView2 ?????Windows 11 ????Windows 10 ?????????? 100MB??" },
        ...
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm text-gray-300">
              OpenClaw ????????
              <span className="font-mono text-brand-400 ml-1.5">{installDir}</span>
            </div>
